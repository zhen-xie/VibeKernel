...
int main(int argc, char** argv) {
  try {
    ...
    cudaGraphExec_t graph = build_graph(a, b, gc, o); auto baseline = [&] { run_baseline(a, b, bc, o); }; auto graph_launch = [&] { CUDA_CHECK(cudaGraphLaunch(graph, 0)); }; auto mpk = [&] { mpk_independent_gemm_kernel<<<workers, o.m * o.n>>>(a, b, mc, o.m, o.n, o.k, o.tasks, q); CUDA_CHECK(cudaGetLastError()); }; auto cublas = [&] { run_cublas_batched(handle, a_array, b_array, c_array, o); };
    baseline(); graph_launch(); mpk(); cublas(); CUDA_CHECK(cudaDeviceSynchronize()); std::vector<float> hbc(c_count), hgc(c_count), hmc(c_count), hcbc(c_count); CUDA_CHECK(cudaMemcpy(hbc.data(), bc, c_count * sizeof(float), cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hgc.data(), gc, c_count * sizeof(float), cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hmc.data(), mc, c_count * sizeof(float), cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hcbc.data(), cbc, c_count * sizeof(float), cudaMemcpyDeviceToHost)); float const ge = max_abs(hbc.data(), hgc.data(), c_count), me = max_abs(hbc.data(), hmc.data(), c_count), ce = max_abs(hbc.data(), hcbc.data(), c_count); std::printf("correctness: graph=%g mpk=%g cublas=%g\n", ge, me, ce); if (ge != 0 || me != 0 || ce > 1e-4f) throw std::runtime_error("backend outputs differ");
    report("baseline <<<>>>", benchmark(baseline, o.warmup, o.repeats)); report("cudaGraphLaunch", benchmark(graph_launch, o.warmup, o.repeats)); report("mpk persistent", benchmark_mpk(mpk, q, o.warmup, o.repeats)); report("cuBLAS", benchmark(cublas, o.warmup, o.repeats));
    std::printf("Note: cuBLAS is one cublasSgemmBatched call using the column-major transpose formulation; MPK uses only an independent-task queue.\n"); CUBLAS_CHECK(cublasDestroy(handle)); CUDA_CHECK(cudaGraphExecDestroy(graph)); CUDA_CHECK(cudaFree(a)); CUDA_CHECK(cudaFree(b)); CUDA_CHECK(cudaFree(bc)); CUDA_CHECK(cudaFree(gc)); CUDA_CHECK(cudaFree(mc)); CUDA_CHECK(cudaFree(cbc)); CUDA_CHECK(cudaFree(q)); CUDA_CHECK(cudaFree(a_array)); CUDA_CHECK(cudaFree(b_array)); CUDA_CHECK(cudaFree(c_array));
