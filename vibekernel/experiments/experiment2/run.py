...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--backend', choices=('all','cublaslt_gemm','triton_large_gemm','persistent_gemm'),default='all'); p.add_argument('--shape-index',type=int,default=1); p.add_argument('--warmup',type=int,default=100); p.add_argument('--iterations',type=int,default=1000); p.add_argument('--cutlass-path'); p.add_argument('--output',type=Path); a=p.parse_args()
 ...
 for name in names:
  ...
  try:
   b.prepare(problems); correct=validate_outputs(b.run(),expected); assert correct['correct'], json.dumps(correct); x=measure_backend(b,total_flops(problems),a.warmup,a.iterations); d=x.to_dict(); d.update(backend=name,correctness=correct,metadata=b.metadata()); out.append(d); print(f'{name:20s} median={x.median_us:.3f} us TFLOPS={x.tflops:.3f}')
  finally: b.close()
